...
UPLOAD_DIRECTORY = "/app/uploads/" # considering docker container
    ...
    if(not meeting_instance.summary_path):
        ...
        try:
            if (not meeting_instance.transcription_path):
                ...
                meeting_instance.transcription_path = transcription_path
...
async def download_summary(instance_id: int):
    ...
    raise HTTPException(
        status_code=404,
        detail=f"The summary of media with id {instance_id} was not computed yet"        
    )
